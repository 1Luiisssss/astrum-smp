...
MC_VERSION         = "1.21.11"
LOG_CHANNEL_ID     = int(os.getenv("LOG_CHANNEL_ID", "0"))  # <-- agrega LOG_CHANNEL_ID en config.env

# ─── Anti-Raid / Anti-Spam ────────────────────────────────────────────────────
from collections import defaultdict
import time

SPAM_LIMITE_MENSAJES  = 5    # mensajes máximos...
SPAM_VENTANA_SEGUNDOS = 5    # ...en esta ventana de tiempo (segundos)
SPAM_CANALES_DISTINTOS = 3   # si spamea en X canales distintos = raid

_spam_contador: dict = defaultdict(list)
_spam_canales:  dict = defaultdict(set)
_spam_mensajes: dict = defaultdict(list)
_baneados:      set  = set()
# ─────────────────────────────────────────────────────────────────────────────
...
async def on_message(message):
    ...
    if uid not in _baneados:
        _spam_contador[uid].append(ahora)
        _spam_canales[uid].add(message.channel.id)
        _spam_mensajes[uid].append(message)

        _spam_contador[uid] = [t for t in _spam_contador[uid] if ahora - t < SPAM_VENTANA_SEGUNDOS]

        mensajes_recientes = len(_spam_contador[uid])
        canales_distintos  = len(_spam_canales[uid])

        if mensajes_recientes >= SPAM_LIMITE_MENSAJES or canales_distintos >= SPAM_CANALES_DISTINTOS:
            _baneados.add(uid)
            motivo = "Anti-Raid: spam en múltiples canales" if canales_distintos >= SPAM_CANALES_DISTINTOS else "Anti-Spam: mensajes repetitivos"

            for msg in _spam_mensajes[uid]:
                try:
                    await msg.delete()
                except Exception:
                    pass
            ...
            try:
                await message.guild.ban(message.author, reason=motivo, delete_message_days=1)
            except Exception:
                pass

            log_ch = bot.get_channel(LOG_CHANNEL_ID)
            if log_ch:
                embed = discord.Embed(title="🚨 Usuario Baneado — Anti-Raid", color=discord.Color.red(), timestamp=datetime.now(timezone.utc))
                embed.add_field(name="Usuario",  value=f"{message.author} (`{uid}`)", inline=False)
                embed.add_field(name="Motivo",   value=motivo, inline=False)
                embed.add_field(name="Mensajes", value=f"{mensajes_recientes} en {SPAM_VENTANA_SEGUNDOS}s", inline=True)
                embed.add_field(name="Canales",  value=str(canales_distintos), inline=True)
                embed.set_footer(text="AstrumSMP Anti-Raid")
                await log_ch.send(embed=embed)

            _spam_contador.pop(uid, None)
            _spam_canales.pop(uid, None)
            _spam_mensajes.pop(uid, None)
            return
    # ── Fin Anti-Raid ──────────────────────────────────────────────────────
